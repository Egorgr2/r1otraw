...
function createBot() {
  ...
  bot.use((ctx, next) => {
    const user = ctx.from;
    const userInfo = user ? (user.username ? `@${user.username}` : `ID: ${user.id}`) : 'Unknown user';
    console.log(`Received update from ${userInfo}:`, JSON.stringify(ctx.update, null, 2));
    return next();
  });
  ...
  bot.command("start", async (ctx) => {
    const user = ctx.from;
    const userInfo = user ? (user.username ? `@${user.username}` : `ID: ${user.id}`) : 'Unknown user';
    console.log(`📩 /start command from user ${userInfo}`);
    ...
    try {
      ...
      console.log(`✅ /start command handled successfully for user ${userInfo}`);
    } catch (error) {
      console.error(`❌ Error handling /start for user ${userInfo}:`, error);
    }
  });
  ...
  bot.command("help", async (ctx) => {
    const user = ctx.from;
    const userInfo = user ? (user.username ? `@${user.username}` : `ID: ${user.id}`) : 'Unknown user';
    console.log(`📩 /help command from user ${userInfo}`);
    const shopName = process.env.NEXT_PUBLIC_SHOP_NAME ?? "Resale Shop";
    const webAppUrl = process.env.WEBAPP_URL;
    ...
    try {
      ...
      console.log(`✅ /help command handled successfully for user ${userInfo}`);
    } catch (error) {
      console.error(`❌ Error handling /help for user ${userInfo}:`, error);
    }
  });
  ...
  bot.on("message:text", async (ctx) => {
    const user = ctx.from;
    const userInfo = user ? (user.username ? `@${user.username}` : `ID: ${user.id}`) : 'Unknown user';
    console.log(`📩 Text message from user ${userInfo}: "${ctx.message.text}"`);
    const webAppUrl = process.env.WEBAPP_URL;
    const shopName = process.env.NEXT_PUBLIC_SHOP_NAME ?? "Resale Shop";
    ...
    try {
      ...
      console.log(`✅ Text message handled successfully for user ${userInfo}`);
    } catch (error) {
      console.error(`❌ Error handling text message for user ${userInfo}:`, error);
    }
  });
